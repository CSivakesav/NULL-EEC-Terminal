...
export const CHANNEL = "NULL_EEC_HOVER_v1" as const;
const VISUAL_EDIT_MODE_KEY = "null_eec_visual_edit_mode" as const;
const FOCUSED_ELEMENT_KEY = "null_eec_focused_element" as const;
...
const postMessageDedup = (data: any) => {
  try {
    ...
    if (key === _nullEecLastMsg) return; // identical – drop
    _nullEecLastMsg = key;
  } catch {
...
// Helper to parse data-null-eec-id to extract file path, line, and column
const parseNullEecId = (
  nullEecId: string
): { filePath: string; line: number; column: number } | null => {
  // Format: "filepath:line:column"
  const parts = nullEecId.split(":");
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-null-eec-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-null-eec-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the null-eec ID from the element to ensure we're working with the right one
    const nullEecId = element.getAttribute("data-orchids-id");
    if (!nullEecId) return;
    ...
    if (newText !== oldText) {
      const parsed = parseNullEecId(nullEecId);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    // Find ALL elements with the same null-eec ID
    const allMatchingElements = document.querySelectorAll(
      `[data-orchids-id="${nullEecId}"]`
    ...
    const existingStyles = appliedStylesRef.current.get(nullEecId) || {};
    appliedStylesRef.current.set(nullEecId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(nullEecId);
  ...
  const flushImageSrcChange = () => {
    ...
    const nullEecId = imgElement.getAttribute("data-orchids-id");
    if (!nullEecId) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "NULL_EEC_STYLE_UPDATE") {
      ...
      } else if (e.data?.type === "NULL_EEC_IMAGE_UPDATE") {
  ...
  useEffect(() => {
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      if (hit) {
        ...
        // Find ALL other elements with the same null-eec ID and show hover boxes
        const allMatchingElements = document.querySelectorAll(
          `[data-orchids-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
